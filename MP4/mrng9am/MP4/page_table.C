...
PageTable::PageTable()
{
    ...
    page_directory = (unsigned long *)(process_mem_pool->get_frames(PT_Count) * PAGE_SIZE);
    // Page Table creation for shared address space between 2MB to 4MB but stored in process memory pool this time
    unsigned long *free_mapped_pt = (unsigned long *)(process_mem_pool->get_frames(PT_Count) * PAGE_SIZE);
    ...
    // LSB - Present(1)/Absent(0) ( decimal 1)
    // LSB + 1 - Write(1)/Read(0) (decimal 2)
    // LSB + 2 - User(1)/Kernel(0)(decimal 4)
...
void PageTable::handle_fault(REGS *_r)
{
    ...
    unsigned long dir_offset = fault_address >> 22;           // moving the first 10 bits which is the directory offset to LSB
    unsigned long tab_offset = (fault_address >> 12) & 0x3FF; // moving the middle 10bits to LSB(table offset) and then removing the leading 10bits (direc offset)
    // temporary page directory pointer
    ...
    if ((error_code & 1) == 0)
    {
        // identify if the address is legitimate
        VMPool **temp_pool = current_page_table->temp_vm_pool;
        int looper = 0;
        while (looper < current_page_table->vm_pool_count)
        {
            if (temp_pool[looper] != NULL)
            {
                if (temp_pool[looper]->is_legitimate(fault_address))
                {
                    flag = true; // flag will return true only if address is legitimate and the page fault will be handled
                    break;
                }
            }
            looper++;
        ...
        if (flag)
        {
            if ((temp_page_dir[dir_offset] & 0x1) == 0)
            {
                ...
                temp_page_dir[dir_offset] = (unsigned long)((process_mem_pool->get_frames(PT_Count) * PAGE_SIZE) | 3);
                ...
                page_table[tab_offset] = (PageTable::process_mem_pool->get_frames(PT_Count) * PAGE_SIZE);
...
void PageTable::register_pool(VMPool *_vm_pool)
{
    ...
    else
    {
        ...
        vm_pool_count += 1; // update total vm pool count
...
void PageTable::free_page(unsigned long _page_no)
{
    ...
    dir_offset = (dir_offset << 12) | (0xFFC00000);
    ...
    unsigned long *temp_pt = (unsigned long *)(dir_offset);

    unsigned long target_frame = temp_pt[tab_offset] / Machine::PAGE_SIZE;
    // release required frame
    process_mem_pool->release_frames(target_frame);

    temp_pt[tab_offset] = 2;
    // flush tlb
}
